...
public class Journey_Details extends JFrame{  //Forth
	...
	String[] items1 =  {"BANGLORE", "MUMBAI", "CHENNAI", "PATNA","DELHI","HYDERABAD"};
	JComboBox comboBox = new JComboBox(items1);
	comboBox.setBounds(150, 100, 150, 27);
	add(comboBox);
		
		
	String[] items2 =  {"BANGLORE", "MUMBAI", "CHENNAI", "PATNA","DELHI","HYDERABAD"};
	...
	Show.addActionListener(new ActionListener(){
		...
		try{
                    ...
                    String str;
                    ResultSet rs=c.s.executeQuery( str = "select pnr_no,ticket_id,f_code,jny_date,jny_time,src,dst from reservation where src= '"+src+"' and dst = '"+dst+"'")
                    ;
                    table.setModel(DbUtils.resultSetToTableModel(rs));
                    if(rs.next()){
                        JOptionPane.showMessageDialog(null,"find Flights between Source and Destination");
                    }else{
                        JOptionPane.showMessageDialog(null,"find Flights between Source and Destination");
